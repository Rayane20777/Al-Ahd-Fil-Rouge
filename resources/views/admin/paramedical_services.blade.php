                ...
                <div class="bg-white border border-gray-100 shadow-md shadow-black/5 p-6 rounded-md">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full min-w-[540px]" data-tab-for="order" data-page="active">
                            ...
                            <tbody>
                                @foreach ($paramedicalServices as $paramedical_service)
                                <tr>
                                    
                                    <td class="py-2 px-4 border-b border-b-gray-50">
                                        <span class="text-[13px] font-medium text-gray-400">{{$paramedical_service->name}}</span>
                                    </td>
                                    <td class="py-2 px-4 border-b border-b-gray-50">
                                        <span class="text-[13px] font-medium text-gray-400">{{$paramedical_service->description}}</span>
                                    </td>
                                    <td class="py-2 px-4 border-b border-b-gray-50">
                                        <span class="inline-block p-1 rounded bg-blue-500/10 text-blue-500 font-medium text-[12px] leading-none">Edit</span>
                                        <form action="{{ route('paramedical_services.destroy', $paramedical_service->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
